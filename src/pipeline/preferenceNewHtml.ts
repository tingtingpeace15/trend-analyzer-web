...
mk('c13b',{type:'doughnut',data:{labels:acSegments.map(function(r){return r.name}),datasets:[{label:'金额占比',data:acSegments.map(function(r){return r.amount}),backgroundColor:acSegments.map(function(r){return acceptSegColor(r.name)}),borderColor:'#fff',borderWidth:1}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'right'},tooltip:{callbacks:{label:function(c){var total=c.dataset.data.reduce(function(a,b){return a+n(b)},0);var r=acSegments[c.dataIndex]||{};return c.label+': '+money(c.raw)+' ('+pct(c.raw,total)+') / 数量占 '+acceptPct(r.qty_share)}}}}}});
