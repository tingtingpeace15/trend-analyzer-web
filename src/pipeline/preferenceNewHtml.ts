...
.designer-cat-chart{border:1px solid var(--line);border-radius:8px;background:#fff;padding:8px 8px 0}
.designer-cat-chart .ch{height:520px}
...
function designerCategoryBoard(){return '<div class="designer-cat-board"><div class="co-filter-controls designer-cat-controls"><label>设计师品牌<div id="designerCatDesigner" class="pf-multi"></div></label></div><div class="designer-cat-summary" id="designerCatSummary"></div><div class="designer-cat-chart"><div class="ch"><div id="designerCatPie" class="echart"></div></div></div></div>'}
...
function designerCategoryOptionRows(){var by={};designerCategoryRows().forEach(function(r){var name=r.designer||'';if(!name)return;if(!by[name])by[name]={name:name,amount:0,qty:0};by[name].amount+=n(r.amount);by[name].qty+=n(r.qty)});return Object.keys(by).map(function(k){return by[k]}).filter(function(r){return n(r.amount)>0||n(r.qty)>0}).sort(function(a,b){return n(b.amount)-n(a.amount)||n(b.qty)-n(a.qty)})}
...
function renderDesignerCategorySummary(rows,total){var selected=multiValues(DESIGNER_CAT_FILTER,'designer'),scope=selected.length?selected.join(' / '):'全部设计师品牌',top=rows[0]||{},el=document.getElementById('designerCatSummary');if(!el)return;el.innerHTML='<div><span>当前范围</span><b>'+esc(scope)+'</b></div><div><span>销售额合计</span><b>'+money(total)+'</b></div><div><span>品类数</span><b>'+fmt(rows.length)+'</b></div><div><span>Top品类</span><b>'+esc(top.category||'-')+' '+(top.share?acceptPct(top.share):'0.0%')+'</b></div>'}
function renderDesignerCategoryChart(){refreshDesignerCategoryControls();var rows=designerCategoryChartRows(),total=rows.reduce(function(s,r){return s+n(r.amount)},0);renderDesignerCategorySummary(rows,total);mk('designerCatPie',{type:'doughnut',data:{labels:rows.map(function(r){return r.category}),datasets:[{label:'销售额',data:rows.map(function(r){return r.amount}),backgroundColor:rows.map(function(r,i){return C[i%C.length]})}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'right'},tooltip:{trigger:'item',callbacks:{label:function(c){var r=rows[c.dataIndex]||{};return '销售额: '+money(c.raw)+' / 占比 '+acceptPct(r.share)},afterBody:function(ctx){var r=rows[ctx[0].dataIndex]||{};return ['品类 '+esc(r.category||'-'),'销售额 '+money(r.amount),'销量 '+fmt(r.qty)+'件','占当前范围 '+acceptPct(r.share)]}}}}}})}
...
document.getElementById('s-br').innerHTML='<div class="tip"><b>品牌/风格洞察：</b>本页只保留标准可视化图表：品牌金额/销量、设计师品牌金额/销量、设计师品类销售额占比、销售额Top20客户品牌偏好、品类-品牌/设计师品牌贡献值。</div><div class="g">'
+card('品牌金额与销量',chart('c11',true))
+card('销售额Top20客户品牌偏好',wideChart('c11cust'),true)
+card('设计师品牌金额与销量',brandTallChart('c12'))
+card('设计师品类销售额占比',designerCategoryBoard(),true)
